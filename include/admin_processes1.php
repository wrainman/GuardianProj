...
class Admin_Process {
	...
	function Register($post, $process) {
		...
		if (isset($process)) {
			...
			$this -> query("INSERT INTO " . DBTBLE . " (first_name, last_name, email_address, username, password, info) VALUES ('$first_name', '$last_name', '$email_address', '$username', '" . sha1($pass1) . "', '" . htmlspecialchars($info) . "')");
	...
	function edit_pass($POST, $edit) {
		...
		if (isset($edit)) {
			...
			$this -> query("UPDATE " . DBTBLE . " SET password = '" . sha1($pass1) . "' WHERE userid = '$userid'");
